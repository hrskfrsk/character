...
// 現在のキャラクターを表示（Next.js版にリダイレクト）
async function displayCurrentCharacter() {
  try {
    // まずキャラクターを保存（既存の場合は更新）
    await saveCurrentCharacter();
    
    // 保存されたキャラクターIDが存在する場合はNext.js版を使用
    if (currentCharacterId) {
      // Next.js版の高速表示ページにリダイレクト
      const nextjsUrl = `http://localhost:3000/character/${currentCharacterId}`;
      window.open(nextjsUrl, '_blank');
      characterManager.showNotification("高速表示ページを開きました", "success");
    } else {
      // 保存に失敗した場合は従来の方法でフォールバック
      ...
      characterManager.showNotification("従来版で表示しました（保存してから再度お試しください）", "info");
    }
  } catch (error) {
    ...
    const encoded = btoa(unescape(encodeURIComponent(JSON.stringify(data))));
    window.open(`character.html?data=${encoded}`, '_blank');
    characterManager.showNotification("エラーのため従来版で表示しました", "warning");
  }
}
...
function getFormData() {
  ...
  // 装備データを追加
  if (typeof window.collectEquipmentData === 'function') {
    data.equipment = window.collectEquipmentData();
  }
...
function setFormData(data) {
  ...
  requestAnimationFrame(() => {
    ...
    // 装備データの復元（フレーム分離で高速化）
    setTimeout(() => {
      if (data.equipment && typeof window.restoreEquipmentData === 'function') {
        window.restoreEquipmentData(data.equipment);
      }
      
      // 追加技能復元後にクリーンアップを実行（さらに分離）
      setTimeout(() => {
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function showLoadingIndicator() {
    const existingLoader = document.getElementById('character-loader');
    if (!existingLoader) {
      const loader = document.createElement('div');
      loader.id = 'character-loader';
      loader.style.cssText = `
        ...
        gap: 10px;
      `;
      loader.innerHTML = '<div style="width:20px;height:20px;border:2px solid #fff;border-top:2px solid transparent;border-radius:50%;animation:spin 1s linear infinite;"></div> キャラクターを読み込み中...';
  ...
  // コンポーネント読み込み完了を待つ最適化された関数
  function waitForComponentsAndLoad(callback) {
    let retryCount = 0;
    const maxRetries = 50; // 5秒まで待機
    
    function checkAndLoad() {
      // 主要コンポーネントの存在確認
      const characterForm = document.getElementById('chara-form');
      const equipmentSection = document.getElementById('equipment-section');
      
      if (characterForm && equipmentSection) {
        callback();
      } else if (retryCount < maxRetries) {
        retryCount++;
        setTimeout(checkAndLoad, 100);
      } else {
        console.warn('Components loading timeout, proceeding anyway');
        callback();
      }
    }
  ...
  if (shareId) {
    ...
    waitForComponentsAndLoad(async () => {
      try {
