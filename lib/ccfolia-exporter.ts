...
export function generateCcfoliaCharacterData(
  ...
  // メモ（基本情報）
  const memo = `${character.character_name_kana || name}
${character.job || "職業"} | ${character.sex || character.gender || "性別"} | ${
    character.age || "年齢"
  } | ${character.height || "身長"}
(PL:${character.player_name || "プレイヤー"})`.trim();
  ...
  // キャラクターカラー
  const color =
    character.character_color_code || character.ui_theme_color || "#22c6d8";
...
CBRB(${character.kick_total || 0},${
      character.martial_arts_total || 0
    }) キック+MA
CCB<=${character.punch_total || 0} こぶし
CBRB(${character.punch_total || 0},${
      character.martial_arts_total || 0
    }) こぶし+MA
CCB<=${character.headbutt_total || 0} 頭突き
CBRB(${character.headbutt_total || 0},${
      character.martial_arts_total || 0
    }) 頭突き+MA`;
    ...
    for (let i = 1; i <= 50; i++) {
      ...
      if (skillName) {
        const skillTotal =
          (parseInt(character[`additional_combat_${i}_initial`] as string) ||
            1) +
          (parseInt(character[`additional_combat_${i}_job`] as string) || 0) +
          (parseInt(character[`additional_combat_${i}_interest`] as string) ||
            0) +
          (parseInt(character[`additional_combat_${i}_growth`] as string) ||
            0) +
...
CCB<=${character.spot_hidden_total || 0} 目星`;
    ...
    for (let i = 1; i <= 50; i++) {
      ...
      if (skillName) {
        const skillTotal =
          (parseInt(
            character[`additional_exploration_${i}_initial`] as string
          ) || 1) +
          (parseInt(character[`additional_exploration_${i}_job`] as string) ||
            0) +
          (parseInt(
            character[`additional_exploration_${i}_interest`] as string
          ) || 0) +
          (parseInt(
            character[`additional_exploration_${i}_growth`] as string
          ) || 0) +
          (parseInt(character[`additional_exploration_${i}_other`] as string) ||
            0);
        palette += `\nCCB<=${skillTotal} ${skillName}`;
      }
    }
...
CCB<=${character.disguise_total || 0} 変装`;
    ...
    for (let i = 1; i <= 50; i++) {
      ...
      if (skillName) {
        const skillTotal =
          (parseInt(character[`additional_action_${i}_initial`] as string) ||
            1) +
          (parseInt(character[`additional_action_${i}_job`] as string) || 0) +
          (parseInt(character[`additional_action_${i}_interest`] as string) ||
            0) +
          (parseInt(character[`additional_action_${i}_growth`] as string) ||
            0) +
...
CCB<=${character.mother_tongue_total || 0} 母国語`;
    ...
    for (let i = 1; i <= 50; i++) {
      ...
      if (skillName) {
        const skillTotal =
          (parseInt(
            character[`additional_negotiation_${i}_initial`] as string
          ) || 1) +
          (parseInt(character[`additional_negotiation_${i}_job`] as string) ||
            0) +
          (parseInt(
            character[`additional_negotiation_${i}_interest`] as string
          ) || 0) +
          (parseInt(
            character[`additional_negotiation_${i}_growth`] as string
          ) || 0) +
          (parseInt(character[`additional_negotiation_${i}_other`] as string) ||
            0);
        palette += `\nCCB<=${skillTotal} ${skillName}`;
      }
    }
...
CCB<=${character.history_total || 0} 歴史`;
    ...
    for (let i = 1; i <= 50; i++) {
      ...
      if (skillName) {
        const skillTotal =
          (parseInt(character[`additional_knowledge_${i}_initial`] as string) ||
            1) +
          (parseInt(character[`additional_knowledge_${i}_job`] as string) ||
            0) +
          (parseInt(
            character[`additional_knowledge_${i}_interest`] as string
          ) || 0) +
          (parseInt(character[`additional_knowledge_${i}_growth`] as string) ||
            0) +
          (parseInt(character[`additional_knowledge_${i}_other`] as string) ||
            0);
        palette += `\nCCB<=${skillTotal} ${skillName}`;
      }
    }
